...
| **Server context = same projection** | `SubjectServerContext` binds a realm by server id (proxy backends). Delegates through `SubjectServerContexts` → `SubjectWorldContexts`; no separate storage. |
...
| `user.inServer("lobby").addPermission(...)` | Server-bound convenience on proxies (same namespace as `inWorld`) |
...
`WorldManager.count()` includes backend world-inheritance nodes that may not map to a loaded Bukkit/Proxy dimension. **World** in PEX is a permission namespace, not strictly “loaded world.” On proxies, realm names are **backend server ids** — there is no Bukkit world dimension. Use `inServer(server)` in proxy plugins; `inWorld(server)` binds the same namespace.
