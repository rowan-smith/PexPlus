...
public class PermissionUser extends PermissionEntity {
     ...
     *            if true then descendant groups of the given group would be checked
     *            too
     ...
     * @param worldName
     * @param checkInheritance
     *            if true than descendant groups of specified group would be checked
     *            too
     * @return true on success, false otherwise
     */
    ...
    protected void updateTimedGroups() {
        ...
        for (Map.Entry<String, String> ent : removeGroups) {
            ...
            if (isDebug()) {
                manager.getLogger().log(Level.INFO, ent.getValue() != null
                        ? "Timed group '{0}' in world '{1}' expired from user {2}/{3}"
