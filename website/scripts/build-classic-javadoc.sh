...
OUT_DIR="${WEBSITE_ROOT}/static/apidocs/${OUT_VERSION}"
LIBS=/tmp/javadoc-libs

prepare_libs() {
  mkdir -p "$LIBS"
  if [ ! -f "$LIBS/bukkit-1.8-R0.1-20150227.224714-194.jar" ]; then
    mvn -f "$REPO_ROOT/pom.xml" dependency:copy -Dartifact=org.bukkit:bukkit:1.8-R0.1-SNAPSHOT:jar -DoutputDirectory="$LIBS" -q
  fi
  for dep in \
    ...
    if ! ls "$LIBS"/${name}*.jar >/dev/null 2>&1; then
      mvn -f "$REPO_ROOT/pom.xml" dependency:copy -Dartifact="${dep}:jar" -DoutputDirectory="$LIBS" -q
    fi
  done
...
git -C "$REPO_ROOT" worktree add "$WORKTREE" "$TAG" 2>&1
