...
Policy: **new hook plugins use `PermissionsExApi`**. Legacy `ru.tehkode.*` stays frozen. See [README.md](/developers/api) and [ARCHITECTURE.md](/developers/architecture).
...
`WorldManager` is a deprecated view of the same registry (`getWorldManager()` → `getRealmManager()`).

## WorldManager (deprecated)

Same registry as `RealmManager`. Prefer `getRealmManager()`, `Realm`, and `Realms` in new code.
...
See `PermissionEventBus` (`dev.rono.permissions.api.event.PermissionEventBus`) Javadoc:
