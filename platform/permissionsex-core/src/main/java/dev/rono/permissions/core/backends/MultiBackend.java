 ...
 */
public class MultiBackend extends AbstractPermissionBackend {
	...
	public MultiBackend(PermissionManager manager, PEXBackendConfiguration backendConfig) throws PermissionBackendException {
		...
		// Fallbacks
		PEXBackendConfiguration fallbackSection = backendConfig.getConfigurationSection("fallback");
	...
	public List<String> getWorldInheritance(String world) {
		for (PermissionBackend backend : backends) {
			...
			if (potentialRet != null && !potentialRet.isEmpty()) {
				return potentialRet;
			}
		}
	...
	public String diagnosticLabel() {
		String primaryJoin = backends.stream().map(PermissionBackend::diagnosticLabel).distinct().collect(Collectors.joining(" + "));
		String label = primaryJoin.isEmpty() ? "multi" : primaryJoin;
		if (!fallbackBackends.isEmpty()) {
			String fb = fallbackBackends.values().stream().map(PermissionBackend::diagnosticLabel).distinct().collect(Collectors.joining(", "));
			if (!fb.isEmpty()) {
				label = label + " + " + fb + " fallback";
			}
		}
		return label;
	}
