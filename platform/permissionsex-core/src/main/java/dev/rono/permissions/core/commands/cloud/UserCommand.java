...
/**
 * Bare {@code pex user} / {@code pex users} Cloud commands. Registered after {@link UserScopedCommand}
 * so {@code pex user &lt;user&gt;} tab completion is not shadowed by the list command.
 *
 * @param <C> command sender type
 */
public final class UserCommand<C> extends AbstractPexCloudCommand<C> {

    public UserCommand(CoreCloudCommandContext<C> ctx) {
        super(ctx);
    }
