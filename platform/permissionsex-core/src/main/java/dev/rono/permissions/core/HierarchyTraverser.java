...
public abstract class HierarchyTraverser<Return> {
	...
	public Return traverse() {
		...
		while (!entities.isEmpty()) {
			...
			if (visited.contains(current)) {
				if (current.isDebug()) {
					current.getPermissionManager().getLogger().warning("Potential circular inheritance detected involving group " + current.getIdentifier() + " (when performing traversal for entity " + start + ")");
				}
				continue;
			}
			visited.add(current);
			...
			if (traverseInheritance) {
				...
				for (int i = parents.size() - 1; i >= 0; --i) { // Add parents to be traversed in order provided by getParents
					entities.addFirst((PermissionEntity) parents.get(i));
				}
			}
		}
		return ret;
	...
	private Return traverseWorldInheritance(PermissionEntity entity) {
		List<String> worldInheritance = entity.getPermissionManager().getWorldInheritance(world);
		if (worldInheritance.size() > 0) {
			...
			while (!worlds.isEmpty()) {
				...
				if (visitedWorlds.contains(current)) {
					if (entity.isDebug()) {
						entity.getPermissionManager().getLogger().warning("Potential circular inheritance detected with world inheritance for world " + current);
					}
					continue;
				}
				...
				final List<String> nextLevel = entity.getPermissionManager().getWorldInheritance(current);
