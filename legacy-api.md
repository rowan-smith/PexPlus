...
**Status:** Frozen public contract. Baseline commit **`628215f`**. Guarded by `LegacyApiContractTest` in `permissionsex-legacy-api`. Do not add methods to `ru.tehkode.*` — use the [modern API]({{ site.baseurl }}/modern-api/) for new features.

Sample plugin: [`permissionsex-example-legacy-plugin`](https://github.com/{{ site.repo }}/tree/main/plugin/permissionsex-example-legacy-plugin/)
...
| `PermissionEntityEvent` | Still use legacy events on Spigot until [modern event bus]({{ site.baseurl }}/api-roadmap/) exists |

See [API index]({{ site.baseurl }}/integrations/) and [FUTURE.md]({{ site.baseurl }}/api-roadmap/).
