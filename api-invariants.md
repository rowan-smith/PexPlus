...
Policy: **new hook plugins use `PermissionsExApi`**. Legacy `ru.tehkode.*` stays frozen. See [Integrations]({{ site.baseurl }}/integrations/) and [Architecture]({{ site.baseurl }}/architecture/).
...
Future read-only configuration/matcher surfaces may adopt the `PermissionService` name (see [FUTURE.md]({{ site.baseurl }}/api-roadmap/)); until then, refer to these types in implementation docs.
...
7. Update [MODERN_API.md]({{ site.baseurl }}/modern-api/) and this file when behavior or layering changes
...
Track API improvements in [FUTURE.md]({{ site.baseurl }}/api-roadmap/).
