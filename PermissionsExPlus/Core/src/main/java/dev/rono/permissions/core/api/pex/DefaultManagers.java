...
final class DefaultUserManager implements UserManager {
    ...
    public boolean isLoaded(UUID uuid) {
        return uuid != null && manager.getActiveUsers().stream()
                .anyMatch(u -> uuid.toString().equals(u.getIdentifier()));
    }
...
final class DefaultGroupManager implements GroupManager {
    ...
    @Override
    public Group create(String name) throws GroupAlreadyExistsException {
        if (exists(name)) {
    ...
    public void delete(String name) {
        ...
        manager.getGroup(name).remove();
        manager.resetGroup(name);
    }
