...
Sample plugin: [`permissionsex-example-plugin`](https://github.com/{{ site.repo }}/tree/main/plugin/permissionsex-example-plugin/)
...
Package overviews live in `package-info.java` files. Architectural rules: [API_INVARIANTS.md]({{ site.baseurl }}/api-invariants/).
...
**Primary API:** `PermissionsExApi` via `PermissionsEx.getApi()`. Subject operations use managers; `PermissionManager` is the legacy/holder bridge. See [API_INVARIANTS.md]({{ site.baseurl }}/api-invariants/).
...
On Spigot, internal dispatches are translated to legacy Bukkit events. See [LEGACY_API.md — Events]({{ site.baseurl }}/legacy-api/#events).

---

## Planned additions

See [FUTURE.md]({{ site.baseurl }}/api-roadmap/) for remaining gaps (diagnostics, config snapshot, matcher access, cache control).
